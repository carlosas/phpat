 ...
 * @coversNothing
 */
class HasOnlyOnePublicMethodNamedTest extends RuleTestCase
{
    use CreatesPhpFile;

    // All subjects live under this namespace prefix so getRule()'s ClassNamespace selector covers them all
    private const SUBJECT_NS = 'Fixture\ShouldHaveOnlyOnePublicMethodNamed';
    ...
    public function testRuleWithRegexMethod(): void
    {
        ...
        $subject = self::SUBJECT_NS.'\GoodImplementationClassWithOnlyOnePublicMethodNamedWithRegexTest\Subject';
        $file = $this->createPhpFile(<<<'PHP'
            <?php
            namespace Fixture\ShouldHaveOnlyOnePublicMethodNamed\GoodImplementationClassWithOnlyOnePublicMethodNamedWithRegexTest;
            class Subject
            {
                public function __construct() {}
                public function methodWithName(): void {}
